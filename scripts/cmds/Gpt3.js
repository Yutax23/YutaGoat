...
module.exports = {
  ...
  onStart: async function({ api, message, event, args, commandName }) {
    ...
    if (prompt === 'clear') {
        const resetUrl = `http://94.130.129.40:8370/gpt3?prompt=clear&uid=${event.senderID}`;
    ...
    const url = `http://94.130.129.40:8370/gpt3?prompt=${encodeURIComponent(tae)}&uid=${event.senderID}`;

    try {
      const response = await axios.get(url);
      const result = response.data.gpt3;
  ...
  if (args[0] === 'clear') {
    ...
    const url = `http://94.130.129.40:8370/gpt3?prompt=${encodeURIComponent(tubol)}&uid=${event.senderID}`;
    
    try {
        const response = await axios.get(url);
        const content = response.data.gpt3;
